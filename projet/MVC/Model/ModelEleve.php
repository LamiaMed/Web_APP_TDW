<?php
require_once('C:/xampp/htdocs/projet/MVC/Controller/ControllerEleve.php');
class ModelEleve{
    private $dbname="TDW";
    ...
    public function get_articles(){
        $c=$this->connexion_db($this->dbname,$this->host,$this->user,$this->password);
        $req="SELECT titre,description,image FROM article WHERE (eleve='1') order by date desc limit 8";
        $res=$this->requete($c,$req);
        $this->deconnexion($c);
        return $res;
    }
    public function authentification($username,$password){
        $c=$this->connexion_db($this->dbname,$this->host,$this->user,$this->password);
        $req="SELECT user,pass FROM compte WHERE (user='$username' AND pass='$password')";
        $res=$this->requete($c,$req);
        $this->deconnexion($c);
        return $res;
    }
